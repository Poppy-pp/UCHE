...
<template>
	<section>
                ...
                <el-form-item style="float:right;marginRight:0;">
                    <Dropdown trigger="custom" :visible="selectColVisible" @on-click="handleSelectCols" class="selectcol">
                        ...
                        <DropdownMenu slot="list">
                            ...
                            <DropdownItem name="platform"><Checkbox v-model="checked.platform"/>平台来源</DropdownItem>
        ...
        <el-table :max-height="windowOutHeight-260" border :data="listData" highlight-current-row v-loading="listLoading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading">
            ...
            <el-table-column prop="platform" label="平台来源" :formatter="platformFormatter" align="center" v-if="isarray('landv',curTableData)">
            </el-table-column>
            ...
            <el-table-column fixed="right" label="操作" align="center" width="120">
                <template slot-scope="scope">
                    <Button disabled type="primary" shape="circle" size="small" @click="handleDetails(scope.$index, scope.row)" title="关联详情">
                        <Icon type="eye"></Icon>
                    </Button>
    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                curTableData:['name','mobile','landv','platform','associate'],//自定义显示列组
        ...
        methods: {
            ...
            categoryFormatter: function (row, column) {
                return row.category == 'SHARE' ? '微信分享' : row.category == 'SYSTEM' ? '系统添加' : '暂无';
            },
            // 平台来源转换
            platformFormatter: function (row, column) {
                return row.platform == 'C' ? '优车' : row.platform == 'V' ? 'VAS' : '暂无'
            },
            //切换当前页
            ...
            editSubmit: function () {
                this.$refs.editForm.validate((valid) => {
                    if (valid) {
                        ...
                        this.users.push({mobile:this.editForm.remobile,tempname:this.editForm.rename});//添加关联人
                        let para = {
                          vehicleid:this.editForm.vehicleid,
                          users:this.users,
